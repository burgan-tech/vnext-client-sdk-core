...
* **300.** **AuthorizationManager.Init**: Cihaz/Kullanıcı doğrulama sistemi, AuthorizationManager init edilir. Eğer hiç login olunmamışsa `deviceId` ve `InstallationId` ile device token oluşturulur. Giriş yapılmış ise, giriş yapmış son kullanıcının 1FA access token'ı kullanılır. Login sonrası ise zaten 2FA token kullanılır.
* **350.** **Router.Init**: Router initialize edilir. Router mode token seviyesine göre belirlenir:
  ...
  - **2FA+ token**: Config'den `router.defaultMode` (SDI/MDI) alınır. `allowChangeMode: true` ise kullanıcı modu değiştirebilir ve seçim kalıcı olur. User preference'dan değiştirilirse uygulama restart gerektirir.
